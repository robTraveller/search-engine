...
class ParseDOC(ParserSuper):
	def parseContent(self, dicInput):
		...
		try:
			self.exportDOC(inFilePath, outFilePath)
	...
	def exportDOC(self, infile, outfile):
		(fi, fo, fe) = os.popen3('catdoc -w "%s" > "%s"' % (infile, outfile))
		fi.close()
		retval = fo.read()
		erroroutput = fe.read()
		fo.close()
...
def main(argv):
	...
	path = r'//pvgn50863254a/share/DOCTEST/good.doc'
